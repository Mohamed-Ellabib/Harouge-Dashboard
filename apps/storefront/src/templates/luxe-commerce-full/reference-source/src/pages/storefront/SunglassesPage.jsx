...
const bottomNavItems = [
  ...
  { label: 'النظارات', href: '/sunglasses', icon: Glasses, active: true },
  { label: 'الساعات', href: '/watches', icon: Clock },
  { label: 'الأقلام', href: '/pens', icon: PenLine },
  { label: 'الحساب', href: '/account', icon: User },
];
const SUNGLASSES_PRODUCTS_URL = STOREFRONT_PRODUCT_FEEDS.sunglasses;
...
function getColorTone(product) {
  ...
  if (/brown|tortoise|leather|بني|جلد/.test(text)) return 'brown';
  return 'all';
}
...
function getMaterialType(product) {
  ...
  if (/rubber|silicone|nylon|fabric|acetate|مطاط|سيليكون|قماش/.test(text)) return 'silicone';
  return 'all';
}
...
export default function SunglassesPage() {
  ...
  if (initialProductsRef.current === null) {
    const cachedData = getStorefrontFeed(SUNGLASSES_PRODUCTS_URL);
    ...
    async function loadProducts() {
      ...
      try {
        ...
        const data = hasInitialCache
          ? await refreshStorefrontFeed(SUNGLASSES_PRODUCTS_URL)
          : await preloadStorefrontFeed(SUNGLASSES_PRODUCTS_URL);
        ...
        if (error.name !== 'AbortError') {
          console.error('Failed to load sunglasses products', error);
          if (!hasInitialCache) {
            setProducts([]);
            setLoadError(true);
          }
  ...
  useEffect(() => {
    function handleSoftRefresh(event) {
      if (event.detail?.feedUrl !== SUNGLASSES_PRODUCTS_URL) return;
  ...
  return (
    <main className="watches-page sunglasses-page" dir="rtl">
      <div className="watches-page__phone-shell">
        ...
        <section className="watches-page__hero" aria-label="تشكيلة النظارات من علاماتنا التجارية">
        ...
        <section className="watches-page__grid" aria-label="منتجات النظارات" aria-busy={isLoading}>
          ...
          {!isLoading && visibleProducts.length === 0 ? (
            <article className="watches-page__empty">
              ...
              <span>{loadError ? 'تحقق من اتصال الخادم ثم أعد المحاولة.' : hasNoFilterResults ? 'جرّب تغيير خيارات التصفية أو الترتيب.' : 'سيتم عرض النظارات هنا عند إضافتها من لوحة التحكم.'}</span>
            </article>
