  ...
  testSuite: ({ api, getContainer }) => {
    describe("Phase 0.5 security regressions", () => {
      ...
      it("keeps products within the authenticated vendor sales channel", async () => {
        const cookieA = await login("merchant-a@example.test", fixtures.passwordA)
        const cookieB = await login("merchant-b@example.test", fixtures.passwordB)
        const createdA = await api.post(
          "/vendor/products",
          {
            ...
            sales_channels: [
              fixtures.salesChannelA,
              fixtures.salesChannelB,
            ],
          },
        ...
        expect(beforeNewChannel.sales_channels.map((channel) => channel.id)).toEqual([
        ...
        expect(afterNewChannel.sales_channels.map((channel) => channel.id)).toEqual([
          fixtures.salesChannelA.id,
        ])
        ...
        expect(storeBProducts.data.products.map((product) => product.id)).not.toContain(
          productA.id
        )
      ...
      it("revokes every prior session after password changes, resets, and disabling", async () => {
        const sessionA = await login("merchant-a@example.test", fixtures.passwordA)
        const sessionB = await login("merchant-a@example.test", fixtures.passwordA)

        for (const cookie of [sessionA, sessionB]) {
          expect(
            (await api.get("/vendor/me", { headers: { Cookie: cookie } })).status
          ).toBe(200)
        }
