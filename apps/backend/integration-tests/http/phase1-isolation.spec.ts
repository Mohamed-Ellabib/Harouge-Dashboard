  ...
  testSuite: ({ api, getContainer }) => {
    describe("Phase 1 authoritative store isolation", () => {
      ...
      it("rejects missing sessions and missing membership relationships", async () => {
        const missing = await api.get("/vendor/me", { validateStatus: () => true })
      ...
      it("rejects inactive stores and ambiguous active login memberships", async () => {
        ...
        expect(ambiguous.data).toEqual({ message: "Invalid vendor credentials." })
      })
      ...
      it("isolates every implemented product operation and ambiguous ownership", async () => {
        ...
        expect(ambiguousList.data.products.map((product: any) => product.id)).toContain(
          productA.id
        )
      ...
      it("binds public product lists and direct reads to the verified store", async () => {
        ...
        const products = await api.get("/store/products", { headers: headersA })
      ...
      it("returns only owned items through list and direct order access", async () => {
        const cookieA = await loginA()
        const cookieB = await loginB()
        ...
        const { result: mixedOrder } = await createOrderWorkflow(getContainer()).run({
          input: {
            region_id: region.id,
        ...
        const { result: storeBOrder } = await createOrderWorkflow(getContainer()).run({
          input: {
            region_id: region.id,
            email: "store-b-customer@example.test",
            currency_code: "eur",
            sales_channel_id: fixtures.salesChannelB.id,
            status: "pending",
            items: [
        ...
        const listA = await api.get("/vendor/orders", { headers: { Cookie: cookieA } })
        const rowA = listA.data.orders.find((order: any) => order.id === mixedOrder.id)
        ...
        const detailA = await api.get(`/vendor/orders/${mixedOrder.id}`, {
          headers: { Cookie: cookieA },
        })
