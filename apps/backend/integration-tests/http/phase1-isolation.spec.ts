...
import { createVendorSessionToken, VENDOR_SESSION_COOKIE } from "../../src/api/_utils/vendor-auth"
...
import { createSecurityFixtures, type SecurityFixtures } from "../helpers/security-fixtures"
...
medusaIntegrationTestRunner({
  ...
  testSuite: ({ api, getContainer }) => {
    describe("Phase 1 authoritative store isolation", () => {
      ...
      const createProduct = async (cookie: string, title: string, handle: string, price = 1500) =>
        (
          await api.post(
            "/vendor/products",
            { title, handle, status: "published", price, currency_code: "eur" },
      ...
      it("isolates every implemented product operation and ambiguous ownership", async () => {
        ...
        const productA = await createProduct(cookieA, "Phase 1 Product A", "phase-1-product-a")
        ...
        expect(listA.data.products.map((product: any) => product.id)).toEqual([productA.id])
        expect(listB.data.products.map((product: any) => product.id)).toEqual([productB.id])
        ...
        await expect(
          link.create([vendorProductLinkDefinition(fixtures.vendorB.id, productA.id)])
        ).rejects.toThrow(/Cannot create multiple links/)
        const ambiguousList = await api.get("/vendor/products", {
          headers: { Cookie: cookieA }
        })
        expect(ambiguousList.data.products.map((product: any) => product.id)).toContain(productA.id)
        const ambiguousDetail = await api.get(`/vendor/products/${productA.id}`, {
          headers: { Cookie: cookieA },
          validateStatus: () => true
        })
        expect(ambiguousDetail.status).toBe(200)

        const unsupportedDelete = await api.delete(`/vendor/products/${productB.id}`, {
          headers: { Cookie: cookieA },
          validateStatus: () => true
        })
      ...
      it("protects shop fields and centralizes owner versus manager permissions", async () => {
        ...
        expect((await fixtures.marketplace.retrieveVendor(fixtures.vendorB.id)).name).toBe(
      ...
      it("enforces the complete public hostname and key matrix", async () => {
        ...
        const storeA = await resolve("store-a.example.test", fixtures.apiKeyA.token)
        const storeB = await resolve("store-b.example.test", fixtures.apiKeyB.token)
        ...
        expect((await resolve("store-a.example.test", fixtures.apiKeyB.token)).status).toBe(404)
        expect((await resolve("store-b.example.test", fixtures.apiKeyA.token)).status).toBe(404)
        expect((await resolve("unknown.example.test", fixtures.apiKeyA.token)).status).toBe(404)

        const normalized = await resolve("STORE-A.EXAMPLE.TEST.:443", fixtures.apiKeyA.token)
        expect(normalized.status).toBe(200)
        expect(normalized.data.vendor.handle).toBe(fixtures.vendorA.handle)

        const forwarded = await resolve("store-a.example.test", fixtures.apiKeyA.token, {
      ...
      it("binds public product lists and direct reads to the verified store", async () => {
        const cookieA = await loginA()
        const cookieB = await loginB()
        const productA = await createProduct(cookieA, "Public Product A", "public-product-a")
        const productB = await createProduct(cookieB, "Public Product B", "public-product-b")
        const headersA = {
      ...
      it("authorizes whole Orders and rejects mixed-store order creation", async () => {
        const cookieA = await loginA()
        const cookieB = await loginB()
        const productA = await createProduct(cookieA, "Order Product A", "order-product-a")
        const productB = await createProduct(cookieB, "Order Product B", "order-product-b")
        const { result: regions } = await createRegionsWorkflow(getContainer()).run({
          input: {
            regions: [
              {
        ...
        try {
          const created = await createOrderWorkflow(getContainer()).run({
            input: {
              ...orderInput(fixtures.salesChannelA.id, productA, "mixed-order@example.test"),
              items: [
                ...orderInput(fixtures.salesChannelA.id, productA, "mixed-order@example.test")
                  .items,
                {
          ...
          expect(String((error as Error).message)).toMatch(/not available for this Store/i)
        }

        if (mixedOrder) {
          await expect(assignDirectOrderOwnership(getContainer(), mixedOrder)).rejects.toThrow(
            /not available for this Store/i
          )
        }

        const { result: storeAOrder } = await createOrderWorkflow(getContainer()).run({
          input: orderInput(
        ...
        const { result: storeBOrder } = await createOrderWorkflow(getContainer()).run({
          input: orderInput(
        ...
        const rowA = listA.data.orders.find((order: any) => order.id === storeAOrder.id)
        expect(rowA.items.map((item: any) => item.product_id)).toEqual([productA.id])
