...
/** Adds the owner-approved Glow Beauty template without changing commerce authority. */
export class Migration20260831100000 extends Migration {
  ...
  override async down(): Promise<void> {
    this.addSql(`
      ...
      declare existing_constraint text;
      begin
        if exists (select 1 from "storefront_document_revision" where "template_key" = 'glow-beauty') then
          raise exception 'Cannot remove glow-beauty while immutable revisions use it.';
        end if;
        select c.conname into existing_constraint
          from pg_constraint c
         where c.conrelid = 'storefront_document_revision'::regclass
